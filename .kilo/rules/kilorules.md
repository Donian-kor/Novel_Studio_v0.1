# Kilo Code Python Agent Rules

## Thinking Process (For Local LLM Optimization)
- Before making any code changes or running commands, **briefly state your plan and reasoning in 1-2 sentences**. This helps maintain reasoning accuracy in local environments.

## Agent Workflow
- When the user requests a change, first inspect the project structure and all relevant files.
- Use available tools to read, search, edit, and run commands whenever appropriate.
...
- **Environment**: Always check if a virtual environment (`.venv` or `venv`) exists and use it. Never install packages globally.
- **Code Style**: Strictly follow PEP 8 standards (4 spaces indentation, clean naming conventions).
- **Dependency**: When adding new libraries, always update `requirements.txt` or `pyproject.toml` immediately.
...
- If the agent can inspect or modify the file, do it directly.
...
## Completion
After completing a task, report briefly in Korean:
- What was changed
- Which files were changed
- Whether the change was tested (Provide test execution results if available)
- What the user should do next (e.g., how to run the script)
