...
<p>Novel Studio는 <b>기획 → 설정 → 스토리 → 원고 → 연속성 기록 → 연속성 검사</b> 순서로 장편 웹소설을 관리합니다.</p>
<h3>새 프로젝트</h3>
<table>
...
<tr><td>구간별 상세 스토리</td><td>기본 50화 단위의 큰 서사 구간입니다.</td></tr>
...
<tr><td>좌측 메뉴</td><td><b>기획 / 설정 DB / 스토리 / 원고 / 연속성 기록</b>를 전환합니다.</td></tr>
<tr><td>우측 상태 패널</td><td>현재 집필 화의 <b>직전 연속성 기록</b>를 보여줍니다.</td></tr>
...
<tr><td>AI로 생성/보완</td><td>저장된 소설 설계을 근거로 항목을 생성하거나 보완합니다.</td></tr>
...
SEC_END_STATE = """
<h2 id="sec-end-state">7. 연속성 기록</h2>
<p>연속성 기록는 <b>실제 원고에서 다음 화에 꼭 필요한 변화만 추린 연결 정보</b>입니다. 별도의 스냅샷 시스템이 아닙니다.</p>
<table>
...
<p>우측 상태 패널에서 원고를 작성할 때는 현재 화가 아니라 <b>직전 화(N-1)의 종료 상태</b>가 표시됩니다. 예를 들어 398화를 쓰면 397연속성 기록를 봅니다.</p>
<h3>자동 생성 설정</h3>
<p>AI 설정에서 <b>원고 저장 후 AI로 연속성 기록 자동 생성</b>을 켤 수 있습니다. 기본값은 꺼짐이며, 꺼져 있으면 저장은 오직 로컬 파일/DB 저장만 수행합니다.</p>
"""

SEC_CONTINUITY = """
<h2 id="sec-continuity">8. 연속성 검사</h2>
<p>연속성 검사는 별도 기억 시스템이 아니라 <b>작품의 실제 데이터가 서로 맞는지 검증하는 기능</b>입니다.</p>
<p>검사 기준에는 현재 원고, 직전 연속성 기록, 관련 설정 DB, 현재 스토리, 시간축, 사건, 인물 상태 등이 포함됩니다.</p>
...
<tr><td><code>db/story.db</code></td><td>기획, 전체 줄거리, 장기/세부 스토리, 화별 스토리</td></tr>
...
<tr><td><code>db/summary.db</code></td><td>연속성 기록와 연속성 검사 결과</td></tr>
...
<p>프로젝트 설정에서는 총 화수와 목표 글자수뿐 아니라 <b>구간별 상세 스토리</b>과 <b>세부 스토리 구간</b> 크기를 변경할 수 있습니다.</p>
"""
...
<p>v1.5.0에서는 스냅샷을 공식 상태 관리 방식에서 제거했습니다. 다음 화에 필요한 정보는 연속성 기록로 전달합니다.</p>
...
SECTIONS = [
    ...
    ('sec-end-state', '7. 연속성 기록', SEC_END_STATE),
...
<pre>기획 → 설정 DB → 스토리 → 원고 → 연속성 기록 → 다음 화 집필 / 연속성 검사</pre>
<p>작품의 실제 원고가 가장 중요한 정본이며, 연속성 기록는 다음 화에 필요한 최소 연결 정보로 사용됩니다.</p>
"""
