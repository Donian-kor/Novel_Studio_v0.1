...
class MasterDiffService:
    def __init__(self, db: object, ai: object):
        self.db = db
        self.ai = ai

    def propose(self, old: str, new: str, category: str = 'master_plan') -> dict[str, Any]:
        if old == new:
            return {'changed': False, 'summary': '변경 없음', 'changes': []}
        prompt = (
            "기존 설정과 새 설정의 변경점을 JSON 배열로 추출하라. 기존 설정을 기준으로 한다.\n"
        ...
        payload = {
            'changed': True,
            'summary': f'{len(changes)}개 변경 제안',
            'changes': changes,
            'hash_old': sha256(old.encode()).hexdigest(),
            'hash_new': sha256(new.encode()).hexdigest(),
        }
