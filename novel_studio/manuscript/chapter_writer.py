...
class ChapterWriter:
    ...
    def write_stream(self,n,extra=''):
        """실시간 스트리밍 집필: 토큰 조각을 yield한다."""
        target=int(self.project.settings['chapter_chars']); tol=int(self.project.settings['tolerance'])
        return self.ai.generate_stream(write(self.context.build(n,extra),n,target,tol),temperature=.72,max_tokens=max(9000,target*2))
